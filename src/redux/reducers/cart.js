...
const calcularNuevoAmount = (cart) => {
  let nuevoAmount = 0;

  if (cart && cart.length > 0) {
    nuevoAmount = cart.reduce(
      (total, item) => total + item.product.price * item.count,
      0,)
  }
  return nuevoAmount.toFixed(2); // Redondea el resultado a 2 decimales
};
  ...
  switch (type) {
    ...
    case UPDATE_ITEM:
      ...
      case REMOVE_ITEM_SUCCESS:
        const newAmount = calcularNuevoAmount(payload.cart);
        const totalItems = payload.cart.reduce((total, item) => total + item.count, 0);
        return {
          ...state,
          items: payload.cart,
          amount: newAmount,
